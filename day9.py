...
def backtest(prices, signals):
    ...
    returns_std = (
        sum((x - sum(equity_curve)/len(equity_curve))**2 for x in equity_curve)
        / len(equity_curve)
    ) ** 0.5

    sharpe_like = (
        (total_profit / trades) / returns_std
        if trades and returns_std != 0
        else 0
    )
    ...
    print("Sharpe-like:", round(sharpe_like, 3))
    print("============================\n")
