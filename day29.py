...
def price_alert(prices, high=205, low=182):
    """Alert if latest price is above high or below low threshold."""
    latest = prices[-1]
    if latest >= high:
        return f"ALERT: Price ${latest} is ABOVE target ${high}"
    elif latest <= low:
        return f"ALERT: Price ${latest} is BELOW target ${low}"
    return f"Price ${latest} is within normal range"
...
def backtest(prices, signals, capital=10000, risk_pct=0.02):
    """Backtest with trade log and performance stats."""
    ...
    for i in range(len(signals)):
        ...
        if signal == "BUY" and position is None:
            ...
            trades += 1
            print(f"  [{trades}] BUY  at ${entry_price} | Shares: {shares}")
            ...
            trade_log.append(
                f"Trade {trades}: ${entry_price} → ${price} | {result} ${round(profit, 2)}")
            print(
                f"  [{trades}] SELL at ${price:<6} | {result} ${round(profit, 2)}")
            position = None
            running += profit
            if running > peak:
                peak = running
...
def market_summary(prices, returns):
    """Print dashboard with all indicators."""
