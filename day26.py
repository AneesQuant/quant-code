...
def calculate_macd(prices, short=5, long=10):
    """MACD: short MA minus long MA. Positive = bullish, negative = bearish."""
    short_ma = round(sum(prices[-short:]) / short, 2)
    long_ma = round(sum(prices[-long:]) / long, 2)
    macd = round(short_ma - long_ma, 2)
